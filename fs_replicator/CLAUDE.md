...
| `projects` | `id BIGINT` | NewGen projects (`pm/` namespace). Full reload every run. In the main replicator, not a standalone script — projects see growing use and need regular re-sync. **Deletion reconcile:** `merge_rows` is additive-only, so a project deleted in FS (e.g. a destructive `/fs-projectify --refresh` that deletes+recreates) would linger as a ghost with its children. After the merge, `sync_projects` calls `_prune_dead_projects` — diffs the complete live `GET /pm/projects` list against the replica and hard-deletes any project not in it, children first (`project_tasks`→`project_members`→`project_tickets`→`projects`). Guarded against an empty fetch (never prunes on a partial/errored list). Clean-at-source analogue of the deleted-ticket reconcile (Les, 2026-07-01). |
